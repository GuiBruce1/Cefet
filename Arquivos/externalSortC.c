...
int main(int argc, char**argv){
    ...
    for(i = 0; i < N; i++){
        ...
        if (i < resto) {
            tamanho_bloco_atual = divisao + 1;
        } else {
            tamanho_bloco_atual = divisao;
        }

        if (tamanho_bloco_atual == 0) {
        ...
        e = (Endereco*) malloc(tamanho_bloco_atual*sizeof(Endereco));
        if(!e){
        ...
        if(fread(e, sizeof(Endereco), tamanho_bloco_atual, entrada) == tamanho_bloco_atual){
            printf("Lido %d = OK\n", i);
        }
        
        qsort(e, tamanho_bloco_atual, sizeof(Endereco), compara);
        ...
        fwrite(e, sizeof(Endereco), tamanho_bloco_atual, saida);
        fclose(saida);
        printf("Escrito %d = OK\n", i);
        free(e);
    }
